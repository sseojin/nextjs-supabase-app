...
## 준비 작업
- [ ] `date-fns` 라이브러리 설치
- [ ] 프로젝트 폴더 구조 생성 (components/landing, components/calendar 등)
- [ ] 기본 TypeScript 타입 파일 생성 준비

## 랜딩 페이지 구현
- [ ] `app/page.tsx` 생성
- [ ] 웹 제목 (Date Planner) 및 로고 추가
- [ ] 앱 설명 문구 작성 (1-2줄)
- [ ] 로그인 버튼 구현 (→ `/auth/login`)
  - [ ] 버튼 크기: 최소 44x44px (터치 친화적)
  - [ ] 전체 너비 또는 충분한 가로 크기
- [ ] 회원가입 버튼 구현 (→ `/auth/sign-up`)
  - [ ] 로그인 버튼과 동일 크기
- [ ] Tailwind CSS로 모바일 우선 레이아웃 적용
  - [ ] 기본값: 모바일 (375px ~ 480px)
  - [ ] 단일 컬럼, 수직 중심 정렬
  - [ ] 적절한 패딩과 여백 (최소 16px)
- [ ] 랜딩 페이지 스타일링 및 모바일에서 시각 확인
...
## 캘린더 컴포넌트 구현 (모바일 우선)
- [ ] `components/calendar/Calendar.tsx` 생성
  - [ ] 현재 월 표시 (예: "2024년 2월")
  - [ ] 이전/다음 달 버튼 구현
    - [ ] 버튼 크기: 최소 44x44px (터치 친화적)
  - [ ] date-fns로 월 계산 로직 작성
  - [ ] 7x6 날짜 그리드 렌더링
...
- [ ] `components/calendar/DateCell.tsx` 생성
  - [ ] 날짜 숫자 표시
    - [ ] 폰트 크기: 16px 이상
  - [ ] 프로젝트 배지 표시 영역 확보
    - [ ] 배지 높이: 최소 24px
...
- [ ] `components/calendar/CreateProjectModal.tsx` 생성
  - [ ] 모달 기본 구조 (shadcn/ui Dialog 활용)
  ...
  - [ ] 프로젝트 제목 입력 필드
    - [ ] 최소 높이: 44px
    - [ ] 폰트 크기: 16px 이상 (입력 시 줌 방지)
  - [ ] 생성 및 취소 버튼
    - [ ] 각 버튼: 최소 44x44px
    - [ ] 버튼 사이 간격: 8px 이상
  - [ ] 입력값 유효성 검사
    - [ ] 오류 메시지는 입력 필드 아래에 표시
  - [ ] 모달 열고 닫기 상태 관리

- [ ] Calendar 컴포넌트에 모달 통합
  - [ ] 빈 날짜 셀 클릭 시 모달 열기
  - [ ] 날짜 정보를 모달에 전달
...
- [ ] 다른 계정에서 공유 링크 접속 확인
- [ ] 참여하기 버튼 클릭 → project_members 저장 확인
- [ ] 캘린더에서 참여한 프로젝트 파란 배지로 표시 확인
- [ ] 2명 이상 참여 불가 확인 (에러 메시지 표시)
- [ ] 이미 참여한 프로젝트 재참여 불가 확인
...
- [ ] 네비게이션
  - [ ] 바텀 탭 유지 (충분한 공간)
  - [ ] 또는 좌측 사이드바로 선택적 변경 가능

### 웹/데스크톱 (1024px 이상) - 웹 최적화
- [ ] 캘린더 페이지
  - [ ] 여유로운 레이아웃
...
- [ ] 반응형 구현
  - [ ] Tailwind responsive 클래스 활용
    - [ ] **기본값**: 모바일 스타일
    - [ ] `md:` (768px): 태블릿 조정 (좌우 분할 시작)
    - [ ] `lg:` (1024px): 웹/데스크톱 최적화 (사이드바, 호버 효과)
    - [ ] `xl:` (1280px): 초대형 화면 최적화 (여유로운 간격)
